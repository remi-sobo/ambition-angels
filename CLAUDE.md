...
- `/teens` — the games hub (specs/c1d2c9e2-teengamesv1; indexable, standalone chrome). Lists "What Are You Built For" as the hero plus the teen games: Higher Wage is live at `/teens/higher-wage` (pair/guess APIs under `app/api/games/higher-wage/*`, gap rule + signed pair token in `lib/games/` — pay never reaches the client pre-tap, and a CI test asserts it); Never Heard of It is live at `/teens/never-heard-of-it` (daily APIs under `app/api/games/daily/*` — clue 1 + answer hashes only, title never pre-reveal; guesses hash-matched client-side via `lib/games/guess.ts`; `game_daily` calendar human-scheduled at `/admin/careers/daily` with a 3-of-7 job-zone≤3 rule, rolls midnight Pacific); The Cut is built at `/teens/the-cut` (rooms + phone voting + solo, `app/api/games/the-cut/*`, engine in `lib/games/the-cut.ts`, ephemeral `cut_*` tables) but soft-launched to facilitators only — noindex and not linked from the hub until classroom runs prove it. `/ms` and `/ms/*` 308-redirect here (`next.config.mjs`) — old flyer/email links never die.
