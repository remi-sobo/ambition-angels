...
// in person and a header named after them ("Remi's Cockpit").
//
...
export default async function CommandCenter() {
  ...
  const ownerName = displayName ? firstName(displayName) : null;
  const orgName = ctx?.orgName ?? "your organization";

  return (
    <div className="min-h-screen bg-ink">
      <div className="max-w-[1400px] px-4 lg:px-8 py-6 lg:py-8 space-y-6">
        <Greeting name={ownerName ?? "there"} org={orgName} />
        <CeoCockpit ownerName={ownerName} />
      </div>
    </div>
  );
}
