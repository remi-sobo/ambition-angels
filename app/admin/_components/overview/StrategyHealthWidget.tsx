...
function reviewLine(nextReviewAt: string | null, lastReviewAt: string | null): { text: string; tone: string } {
  ...
  if (days === 0) return { text: "Monthly review due today", tone: "text-status-watch-text font-semibold" };
  if (days <= 7) return { text: `Monthly review in ${days}d`, tone: "text-status-watch-text" };
          ...
          <ul className="space-y-2">
            {objectives.map((o) => (
              <li key={o.id} className="flex items-center gap-2.5">
                ...
                {o.kpisOffTrack > 0 && (
                  <span className="text-xs text-expense font-semibold tabular-nums flex-shrink-0">
                    {o.kpisOffTrack} off target
                  </span>
                )}
                <span className="text-xs text-ink-2 flex-shrink-0">{HEALTH_LABEL[o.health] ?? o.health}</span>
              </li>
            ))}
          </ul>
          {headlineKpis.length > 0 && (
            <div className="mt-4 border-t border-hairline pt-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs uppercase tracking-wider text-ink-3 font-semibold">Key measures</span>
                <Link href="/admin/strategic-plan/scorecard" className="text-xs font-semibold text-orange hover:text-orange-mid">
                  Scorecard →
                </Link>
              </div>
              <ul className="space-y-2">
                {headlineKpis.map((k) => (
                  <li key={k.id} className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs text-ink-2 tabular-nums flex-shrink-0 w-20 text-right">
