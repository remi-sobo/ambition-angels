  ...
  return (
    <section className="mb-6">
      <div className="flex items-center justify-between mb-2.5">
        ...
        <Link href="/admin/fundraising/today" className="text-xs font-semibold text-orange hover:text-orange-dark">
          Open in Fundraising →
        </Link>
      </div>
      ...
      <div className="space-y-2">
        {moves.map((m, i) => {
          ...
          const inner = (
            <>
              <div className="flex items-center gap-2 flex-wrap mb-1">
                <span className="text-xs font-bold text-ink-3 tabular-nums">#{i + 1}</span>
                <span className="font-heading font-semibold text-[14px] text-ink-1">{m.name}</span>
                <span className="text-xs text-ink-3 capitalize">{m.stage}</span>
                {m.askAmount ? <span className="text-xs text-ink-3">· {money(Number(m.askAmount))}</span> : null}
                <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full capitalize ${CHANNEL_STYLE[m.channel] ?? CHANNEL_STYLE.other}`}>
                  {m.channel}
                </span>
                {due && <span className="text-xs text-ink-3">· due {due}</span>}
              </div>
          ...
          return (
            <div key={m.id} className="rounded-panel border-hairline bg-surface px-4 py-3">
              {m.constituentId ? (
