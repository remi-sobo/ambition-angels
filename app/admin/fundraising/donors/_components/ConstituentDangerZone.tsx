...
export default function ConstituentDangerZone({
  ...
  return (
    <div className="border-t border-hairline pt-3 flex items-center gap-3 flex-wrap">
      <button
        type="button"
        onClick={toggleArchive}
        disabled={busy !== null}
        className="text-xs font-semibold text-ink-2 hover:text-ink-1 bg-tile hover:bg-tile border-hairline px-3 py-1.5 rounded-full transition-colors disabled:opacity-60"
      >
      ...
      <button
        ...
        className="text-xs font-semibold px-3 py-1.5 rounded-full transition-colors disabled:opacity-40 text-expense border-expense/30 bg-expense-bg hover:bg-expense/10"
      >
        {busy === "delete" ? "…" : "Delete"}
      </button>
      {hasGifts && <span className="text-xs text-ink-3">Delete is disabled. Donor has giving history.</span>}
    </div>
  );
}
