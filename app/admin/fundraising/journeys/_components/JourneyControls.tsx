...
export function NewJourneyForm() {
  ...
  return (
    <form onSubmit={submit} className="bg-tile border-hairline rounded-panel-lg p-4 space-y-4 w-full">
      <div className="flex flex-wrap gap-3">
        ...
        <label className={labelCls}>
          ...
          <select value={trigger} onChange={(e) => setTrigger(e.target.value)} className={inputCls + " w-72"}>
            {TRIGGERS.map(([v, l]) => (<option key={v} value={v} className="bg-tile">{l}</option>))}
          </select>
        </label>
      </div>
      ...
      <div className="space-y-3">
        {steps.map((st, i) => (
          <div key={i} className="border-hairline rounded-control p-3 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs uppercase tracking-wider text-ink-3 font-semibold">Step {i + 1}</span>
        ...
        <button type="button" onClick={addStep} className="text-xs font-semibold text-orange hover:text-orange-dark transition-colors">+ Add step</button>
      </div>
      ...
      <button
        ...
        onClick={async () => {
          const ok = await confirm({
            ...
            body: "Enrollments are removed.",
            confirmLabel: "Delete",
            destructive: true,
          });
          if (ok) void act(() => fetch(`/api/admin/journeys/${id}`, { method: "DELETE" }));
        }}
