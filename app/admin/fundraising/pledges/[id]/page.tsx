      ...
      <div className="max-w-workspace px-4 lg:px-8 py-6 lg:py-8 space-y-6">
        <PageHeader
          ...
          title={
            <span className="flex items-center gap-3 flex-wrap">
              ...
              <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-orange/15 text-orange uppercase tracking-wider">{p.status}</span>
            </span>
          }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          <section className="lg:col-span-4 bg-tile border-hairline rounded-panel-lg p-5 space-y-3">
            ...
            {p.notes && <p className="text-xs text-ink-2 border-t border-hairline pt-3">{p.notes}</p>}
          </section>
                ...
                {payments.map((x) => {
                  ...
                  return (
                    <li key={x.id} className="px-5 py-3 flex items-center gap-4">
                      ...
                      {x.gift_id && (
                        <span className="text-xs text-ink-3">gift recorded</span>
                      )}
