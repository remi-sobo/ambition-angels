...
//
// Extracted at Spec Fundraising F4: the V1 route renders it standalone
// (drawer=false — output byte-identical to the pre-F4 page) and the R1
// research drawer off Donors & Funders renders it with drawer=true (compact
// heading instead of page chrome; the disqualified/active toggle links stay
// on whichever route hosts it via basePath+extraParams). Row links keep
// going to the V1 prospect detail — at F6 that URL 308s to the Donor 360's
// prospect variant, so nothing dies.
...
export default async function ProspectBench({
  searchParams,
  drawer = false,
  basePath = "/admin/fundraising/prospects",
  extraParams = {},
}: {
  ...
  const toggleHref = (show: string | null) => {
  ...
  return (
    <div className={drawer ? undefined : "max-w-[1400px] px-4 lg:px-8 py-6 lg:py-8"}>
      {drawer ? (
        <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
          <div>
            <h2 className={TYPE.cardTitle}>
              {disqualifiedView ? "Bench · Disqualified" : "The Bench"}
            </h2>
