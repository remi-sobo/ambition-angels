...
//
// Extracted at Spec Fundraising F2: the V1 route renders it as-is
// (byte-identical output) and the Donor 360 at donors-funders/[id] renders
// it as the prospect variant of the one URL shape — same screen, both id
// spaces — inside the same ai.prospect_research fence the V1 layout applies.
...
export default async function ProspectProfile({ id, v2 = false }: { id: string; v2?: boolean }) {
  const prospectId = id;
  ...
  return (
    <div className="max-w-5xl px-4 lg:px-8 py-6 lg:py-8 space-y-6">
      <Link
        href={v2 ? "/admin/fundraising/donors-funders?view=prospects" : "/admin/fundraising/prospects"}
