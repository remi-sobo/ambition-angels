...
import ComposeForm from "@/app/admin/finance/reports/_components/ComposeForm";
import ExportControls from "@/app/admin/finance/reports/_components/ExportControls";
import { ARTIFACT_TYPE } from "@/lib/impact/reportExport";
import { TYPE } from "@/lib/admin/typeScale";

// Spec Impact, stage I2 — the Reports builder, the second surface where
// Contract 7's exit gate is a user experience (the N3 pattern verbatim,
// decision 2: shared machinery, Impact's own screen). Two URL-driven states:
//
//   default        — compose a report + recent exports (documents rows,
...
// shipped artifact. The narrative slot renders only an APPROVED reed_drafts
// row (kind 'report_narrative' — the org-wide slot Finance reads too, per
// the spec's ruling; nothing here generates or sends anything on its own).
...
export default async function ImpactReportsPage({
  ...
  searchParams?: { draft?: string; rid?: string; title?: string; keys?: string };
}) {
  ...
  if (searchParams?.draft === "1" && isUuid(searchParams.rid)) {
    ...
    const title = (searchParams.title ?? "").trim().slice(0, 120) || "Impact report";
    ...
    return (
      <div className="max-w-4xl px-4 lg:px-8 py-6 lg:py-8 space-y-6">
        <PageHeader
          eyebrow={
            <Link href="/admin/impact/reports" className="hover:text-ink-1 transition-colors">
              ← Reports
            </Link>
          }
          title={title}
        ...
        <ExportControls
          ...
          exportUrl="/api/admin/impact/reports/export"
  ...
  // ── Default: compose + recent exports ───────────────────────────────────
  ...
  return (
    <div className="max-w-4xl px-4 lg:px-8 py-6 lg:py-8 space-y-6">
      ...
      <ComposeForm
        ...
        basePath="/admin/impact/reports"
        defaultTitle="Impact report"
