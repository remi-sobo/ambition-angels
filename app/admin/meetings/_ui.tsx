...
export const STATUS: Record<FollowUpStatus, { label: string; pill: string; dot: string }> = {
  ...
  none_needed: { label: "No follow-up", pill: "bg-status-neutral-bg text-ink-2 border-hairline", dot: "bg-gray-mid" },
  dismissed: { label: "Dismissed", pill: "bg-status-neutral-bg text-ink-3 border-hairline", dot: "bg-gray-mid" },
};
...
export function MatchCluster({ matched, muted = false }: { matched: MatchedEntity[]; muted?: boolean }) {
  if (matched.length === 0) {
    return <span className="text-xs text-ink-3 italic">unmatched</span>;
  }
  ...
  return (
    <span className="inline-flex items-center gap-2 min-w-0">
      ...
      <span className="inline-flex items-baseline gap-1.5 min-w-0">
        <span className={`text-sm truncate ${muted ? "text-ink-2" : "text-ink-1 font-medium"}`}>{primary.name}</span>
        {rest.length > 0 && <span className="text-xs text-ink-3 shrink-0">+{rest.length}</span>}
      </span>
    </span>
  );
}
...
export function StatusPill({ status }: { status: FollowUpStatus }) {
  ...
  return (
    <span className={`shrink-0 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs uppercase tracking-wider font-semibold border ${s.pill}`}>
