...
// This is the working surface: manual values, status, owner, and notes edit in
...
//
// Extracted at Spec Impact I3 (decision 3, resolved: the scorecard's map row
// rules it merged into Impact → KPIs, and a working surface is absorbed, not
// parked). The V1 route (/admin/strategic-plan/scorecard) renders it
// standalone — output byte-identical to the pre-I3 page — and Impact → KPIs
// embeds the same cards under the catalog hub, editing intact (the client
// islands travel whole). At the I4 cutover the standalone route 308s to
// /admin/impact/kpis.
...
type KpiRow = {
  ...
  goal_id: string | null; objective_id: string | null; notes: string | null;
};

export default async function ScorecardSection({ embedded = false }: { embedded?: boolean }) {
  ...
  // The owner sections render identically in both variants — only the chrome
  // around them differs.
  const body =
    kpis.length === 0 ? (
      <EmptyState
