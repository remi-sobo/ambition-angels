...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: before } = await supabase
    .from("cohorts").select("*").eq("org_id", ctx.orgId).eq("id", params.id).maybeSingle();
  ...
  const { error } = await supabase.from("cohorts").update(update).eq("org_id", ctx.orgId).eq("id", params.id);
  if (error) {
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { error } = await supabase.from("cohorts").delete().eq("org_id", ctx.orgId).eq("id", params.id);
  if (error) {
