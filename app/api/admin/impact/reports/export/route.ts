...
} from "@/lib/impact/reportExport";
import { DOCUMENTS_BUCKET } from "@/lib/documents/config";
import { audit } from "@/lib/audit";

// POST /api/admin/impact/reports/export — the Contract 7 EXIT for a composed
// impact report (Spec Impact I2 — the N3 finance route's exact shape with
// Impact's artifact type; decision 2: shared machinery, parallel exits).
//
...
// the shipped document. This route never writes a waiver and never
// re-implements reports.approve; the documents insert rides the SESSION
// client so documents-write RLS is the authority there.
...
export async function POST(req: NextRequest) {
  ...
  const title =
    (typeof body?.title === "string" && body.title.trim().slice(0, 120)) || "Impact report";
