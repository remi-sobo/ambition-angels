...
interface Props {
  /** Show the teen/adult chooser first. The /teens page skips it. */
  showAudiencePicker?: boolean;
  defaultAudience?: "teen" | "adult";
  /** Analytics source tag, e.g. "career_quiz_page". */
  source?: string;
}
...
export default function CareerQuiz({
  ...
  defaultAudience = "teen",
  source,
}: Props) {
  const initialStage: Stage = showAudiencePicker ? "audience" : "survey";
  const [stage, setStage] = useState<Stage>(initialStage);
  const [audienceMode, setAudienceMode] = useState<"teen" | "adult">(defaultAudience);
  ...
  // Track quiz_started when the quiz mounts (modal open or page visit)
  useEffect(() => {
    trackEvent("quiz_started", source ? { source } : undefined);
  }, [source]);
