...
Received from Remi 2026-09-03. **Recommendations, not decisions** — nothing here is signed. The proposals are staged in `docs/v2-preservation-ledger.md` as `PROPOSED (unsigned)` lines; the gate stays closed until each carries a signed ruling. A codebase review of the draft is appended at the bottom.
...
| R3 comms | **accept, with a gate-consistency caveat** | Three of the tables are **not** zero-row: `email_campaigns` 4, `email_sends` 13, `org_comms_settings` 2 (the latter carries live per-org config including SafeSpace). The gate's own rule says non-zero rows can't be `dormant` without naming the surface that reaches them. The signed line should either accept documented unreachability until Comms builds, or name a read-only holding surface. `email_suppressions` stays `settings` regardless — it is the legally required unsubscribe list. |
...
*Still read-and-report. Nothing signed, nothing built.*
