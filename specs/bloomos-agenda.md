...
Phase 3. Agenda service and Today component. Build the delegation-aware agenda assembler and one shared Today/agenda UI component (day and week). Read through the session client so RLS applies. Commit: `feat(agenda): agenda service and today component`. **[2026-06-25: SHIPPED. `lib/agenda/service.ts` (`getAgenda` — session-client read so RLS scopes to self + delegated; merges /meet bookings deduped on google_event_id), `lib/admin/profile.ts#getDisplayNames` (owner chips), `AgendaList` (client: Day/Week toggle, day grouping, external dot, owner chips when >1 calendar, "synced Xm ago" ochre past 20m + Refresh), `TodayAgenda` (server wrapper). Mounted on the CEO cockpit in place of `ScheduleWidget` (which read the live env token; the new one reads the durable, delegation-aware cache). Verified against Remi's live data: 24 events/next 8d, ~5 today, 0 duplicate bookings. ScheduleWidget still used by OpsPanel until Phase 5.]**

Phase 4. Briefing leads with Today. Restructure the Executive Briefing to lead with the agenda plus today's moves, keep the verdict and exception list below, add the weekly variant. Wire the strategy-vs-calendar cross-reference into the narrative. Commit: `feat(briefing): lead with today agenda and moves`. **[2026-06-25: SHIPPED. `app/admin/briefing/page.tsx` now leads with `<TodayAgenda />` (agenda + week toggle) above the verdict (`NarrativeHero`); pulse, decision feed, fundraising moves and follow-ups stay below. Narrative cross-reference: `narrate.ts` adds a DETERMINISTIC calendar summary to the fact sheet (meetings next 7d, external count, today) — counts only, model never sees raw events — and the system prompt permits a time-vs-strategy cross-reference (e.g., fundraising behind + few external meetings). Verified the summary returns real numbers (18/7d, 12 external, 5 today). Weekly narrative cache deferred per open decision #5; the agenda's week toggle covers the week view. Today's "moves" remain in the existing FundraisingPriorities/FollowUps sections below the verdict.]**

Phase 5. Ops two-lane and sidebar. Render the agenda in two-lane mode (Shannon: hers plus Remi's) in the ops panel. Move the how-to guide to the sidebar footer. Commit: `feat(ops): EA two-lane agenda` and `chore(nav): demote how-to guide to footer`. **[2026-06-25: SHIPPED. `getVisibleOwners()` resolves self + grantors via agenda_delegations (RLS); `AgendaList` gained a `lanes` layout (one column per calendar, shown even when empty so it prompts connecting); `TodayAgenda mode="lanes"` mounted in OpsPanel (replacing ScheduleWidget; kept widget key "schedule" so saved layouts map). How-To Guide removed from Command Center nav and added as a subdued footer link above Settings. Dead ScheduleWidget.tsx removed (getSchedule left as an unused export in the shared sources file). Verified as Shannon: resolves [Shannon, Remi], sees Remi's 39 events + 0 of her own, reads both names. ALL PHASES 1-5 COMPLETE.]**
