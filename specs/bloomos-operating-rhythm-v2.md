...
Status: design, ready for Phase 0 recon. Both blocking decisions resolved (area axis = `category`; time-blocks private-and-persist). Designs both wizards from scratch (the Monday flow does not exist; the `ops_tasks` planning fields are present but unused, exactly one task has a `planned_week`). Borrows the ritual shape of a guided weekly team meeting (orient, walk the areas, walk the days, close out). Nothing here connects to Trellis, Cy, or any SOBO family product; structural inspiration only, per the entity boundary.
