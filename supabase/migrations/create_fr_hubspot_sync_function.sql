-- Ongoing HubSpot → spine sync. Re-runnable + idempotent: called after every
-- hs_* refresh by the sync job (app/api/admin/hubspot/sync). Extends the
-- one-time import_hubspot_to_constituents with granular stage mapping, the
-- HubSpot pipeline, AIG tagging, and (per the working-session decision) gift
-- creation from closedwon deals.
--
-- Decisions (specs/fundraising-v2.md):
--   * HubSpot deals are the gift source of record; payment gifts
--     (Stripe/Givebutter) are deduped against deal-gifts on
--     (constituent, amount, gift_date ±7d) so nothing is counted twice.
--   * Deals already imported as grants are excluded from gift creation.
--   * AIG membership is a constituent tag, not a deal stage.
-- Mapping is mirrored in lib/hubspot/stage-map.ts for the UI.
