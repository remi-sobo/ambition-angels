-- Harden the revenue-schedule "commitment" branch against double-counting a
-- commitment that has already been received as a gift.
--
-- Context: surface_committed_deals_in_revenue_schedule.sql added AIG Member +
-- Pledged deals to v_revenue_schedule as committed inflows. When such a deal is
-- paid it moves to closedwon → it leaves the commitment branch and becomes a
-- gift, so the normal lifecycle never double-counts. BUT a *duplicate* deal can:
-- if the same money exists both as a received gift and as a still-open AIG/
-- Pledged deal (e.g. someone cloned "O'Hara FY26 (AIG Y2)" and only moved one
-- copy to Closed Won), a future-dated duplicate would count as BOTH a gift and a
-- commitment.
--
-- Fix: exclude a commitment row when a gift already matches it on
-- constituent + amount + a ±7-day date window — the same dedup key the sync uses
-- when turning closedwon deals into gifts (create_fr_hubspot_sync_function.sql,
-- step 4). Today this changes no numbers (the only live duplicate is past-dated,
-- so already excluded by expected_close >= current_date); it prevents the
-- double-count if such a duplicate is ever future-dated.
--
...
with (security_invoker = true) as
  ...
  -- committed AIG Member (3448542950) + Pledged (59189578) deals, future-dated,
  -- full value. GUARD: skip any whose money has already landed as a gift
  -- (constituent + amount + ±7 days) so a duplicate deal can't be counted as
  -- both a gift and a commitment.
  select
...
comment on view public.v_revenue_schedule is
  'Canonical dated schedule of expected inflows (pledges, grants, weighted pipeline, committed AIG/Pledged deals, manual). Commitment rows are deduped against received gifts (constituent+amount+-7d) so a duplicate deal cannot count as both. security_invoker — RLS of the underlying tables applies. Never read hs_deals for finance numbers; pipeline + commitments come from opportunities.';
