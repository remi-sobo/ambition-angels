-- Dedupe the HubSpot round-trip: a Bloom-native opportunity pushed OUT to
-- HubSpot records its deal id in external_ids.hubspot_deal (sync-out) but
-- keeps external_source/external_id null — so when the same deal flows back
-- IN, section 3's `on conflict (external_source, external_id)` never fires
-- and the projection inserts a duplicate opportunity (observed: two open
-- "Alan Schroeder" asks, deal 336682424020).
--
-- Fix: new section 3a stamps external_source='hubspot' / external_id onto
-- Bloom-native rows that carry a hubspot_deal stamp BEFORE the deal insert,
-- so the round-tripped deal converges onto the original row (Bloom keeps
-- stage ownership via the existing conflict clause). One row per deal id is
-- stamped (oldest first) and never one whose deal id is already claimed, so
-- the partial unique index on (external_source, external_id) can't trip.
--
-- Everything else is a faithful CREATE OR REPLACE of the live definition
-- (hubspot_projection_external_refs.sql generation).
